...
export const examConnector = {
    ...
    getStudentExamSubmissionStage: async (examId: number, studentId: number): Promise<{ submission_count: number }> => {
        try {
            const response = await api.get(`/api/v1/get_student_exam_submission_stage/${examId}/${studentId}`);
            return response.data;
        } catch (error) {
